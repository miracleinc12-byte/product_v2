...
export async function POST(req: NextRequest) {
  const settings = await getSettings([
    "CRON_SECRET", "GEMINI_API_KEY", "NEWS_API_KEY",
    "UNSPLASH_ACCESS_KEY", "ARTICLE_LENGTH",
  ]);
  ...
  for (const category of targetCategories) {
    try {
      ...
      results.push({ category, title: "-", status: `오류: ${err instanceof Error ? err.message : String(err)}` });
    }
